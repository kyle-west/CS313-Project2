...
} else pool = new Pool(process.env.DATABASE_URL);
...
function post_new_transaction(data, callback) {
   ...
   var q = "INSERT INTO transactions (account_id, user_id, amount, notes) VALUES " +
           "($1, (SELECT id FROM users2 WHERE username = $2), $3, $4) RETURNING id;";
   pool.query(q, [data.account_id, data.username, data.amount, data.notes], (err, res) => {
      if (err)
         console.log("ERROR: " + JSON.stringify(err));
      else {
         console.log("RESULTS: " + JSON.stringify(res));
