...
function validate_user(data, callback) {
   ...
   var q = "SELECT u.id, u.password, u.account_id, a.connection_code FROM users2 u " +
   ...
   pool.query(q, [data.username], (err, res) => {
      if (err)
         ...
         if (found) // avoid null rows reference
...
function update_user_transaction(data, callback) {
   ...
   var q = "UPDATE transactions SET user_id = (SELECT id FROM users2 WHERE username = $1), "
         + "amount = $2, notes = $3, transdate = $6, active = $7 WHERE id = $4 AND account_id = $5 RETURNING id;";
   pool.query(q, [
         data.username,       // $1
         data.amount,         // $2
         data.notes,          // $3
...
function add_user(data, callback) {
   ...
   pool.query(q, [data.username, bcrypt.hashSync(data.password), data.account], (err, res) => {
      if (err)
         console.log("ERROR: " + JSON.stringify(err));
      else {
         console.log("RESULTS: " + JSON.stringify(res));
...
function create_new_user(data, callback) {
   ...
   pool.query("INSERT INTO accounts (name, connection_code) VALUES ($1, $2) "+
              "ON CONFLICT (connection_code) DO UPDATE SET connection_code = $2 RETURNING id;",
...
module.exports = {
   ...
   transaction : {
      ...
      // THE BELOW FUNCTION WAS TAKEN OUT BECAUSE IT WAS TOO DANGEROUS
